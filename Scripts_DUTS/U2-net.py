...
image_path = os.path.join(os.getcwd(),'DUTS-TR/DUTS-TR-Image')
gt_path = os.path.join(os.getcwd(), 'DUTS-TR/DUTS-TR-Mask')
...
while True: 
  if len(os.listdir(image_path))//batch_size == counter+1:
    counter = 0
  if epoch%150 == 0:
    K.set_value(model.optimizer.learning_rate, lr*0.2)
  images, masks = getData(image_path, gt_path, counter, batch_size)
